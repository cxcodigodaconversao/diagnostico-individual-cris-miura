...
const App = () => {
  ...
  const calculateResults = () => {
    ...
    diagnosticoData.areas.forEach((area, areaIndex) => {
      ...
      areaScores.push({ nome: area.nome, percentage: areaPercentage })
  ...
  const renderQuestion = () => {
    const area = diagnosticoData.areas[currentAreaIndex]
    const totalQuestions = diagnosticoData.areas.reduce((sum, a) => sum + a.perguntas.length, 0)
    ...
    return (
      <div className="step-container">
        ...
        <div className="question-card">
          ...
          <div className="question-text">{area.perguntas[currentQuestionIndex]}</div>
        ...
        <div className="results">
          <div className="overall-score">
            ...
            <div className="diagnosis">
              ...
              <div style={{marginBottom: '30px'}}>
                ...
                <div style={{display: 'grid', gridTemplateColumns: '1fr', gap: '15px', marginBottom: '30px'}}>
                  {areaScores.map((area, index) => {
                    let areaStatus = ''
                    let areaColor = ''
                    if (area.percentage >= 80) { areaStatus = 'EXCELENTE'; areaColor = '#27ae60' }
                    else if (area.percentage >= 60) { areaStatus = 'BOM'; areaColor = '#f39c12' }
                    else if (area.percentage >= 40) { areaStatus = 'ATENÇÃO'; areaColor = '#e67e22' }
                    else if (area.percentage >= 20) { areaStatus = 'CRÍTICO'; areaColor = '#e74c3c' }
                    else { areaStatus = 'EMERGENCIAL'; areaColor = '#8e44ad' }
                    return (
                      <div key={index} style={{padding: '20px', background: 'rgba(255,255,255,0.08)', borderRadius: '6px', borderLeft: `4px solid ${areaColor}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                        <div style={{fontFamily: 'Montserrat, sans-serif', fontWeight: 500, color: 'white', fontSize: '1.1em', textAlign: 'left'}}>{area.nome}</div>
                        <div style={{textAlign: 'right'}}>
                          <div style={{fontFamily: 'Readex Pro, sans-serif', fontWeight: 700, fontSize: '1.4em', color: 'white'}}>{area.percentage}%</div>
                          <div style={{fontSize: '0.85em', color: areaColor, fontWeight: 600}}>{areaStatus}</div>
                        </div>
                      </div>
                    )
                  })}
              ...
              <div style={{background: 'rgba(210, 188, 143, 0.1)', padding: '25px', borderRadius: '6px', borderLeft: '4px solid #d2bc8f', margin: '25px 0'}}>
                <h4 style={{color: '#d2bc8f', marginBottom: '20px', fontSize: '1.2em', textAlign: 'center'}}>🦷 Áreas que Precisam Melhorar</h4>
                <ul style={{lineHeight: '1.7', fontSize: '1.0em', paddingLeft: '20px', listStyle: 'none', textAlign: 'left'}}>
                  {diagnosisData.melhorar.map((item, index) => (
                    <li key={index} style={{marginBottom: '8px'}}>🦷 {item}</li>
                  ))}
                </ul>
              </div>
              <div style={{background: 'linear-gradient(135deg, #d2bc8f 0%, #c1a76f 100%)', padding: '30px', borderRadius: '8px', marginTop: '35px', textAlign: 'center', color: '#082033'}}>
                <h4 style={{color: '#082033', marginBottom: '25px', fontSize: '1.4em', fontWeight: 700, textAlign: 'center'}}>Para Alcançar os Resultados Você Precisa de Três Pilares</h4>
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '25px', margin: '25px 0', textAlign: 'left'}}>
